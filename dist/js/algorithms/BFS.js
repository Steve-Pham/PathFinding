function BFS(grid, start_row, start_col, end_row, end_col, allNodes) {
  ...
  let endingNode = grid.getNode(end_col, end_row); //console.log(startingNode.x + ', ' + startingNode.y);
  //console.log(endingNode.x + ', ' +endingNode.y)

  let queue = [];
  queue.push(startingNode);
  startingNode.status = true; // BFS go wide, always look at neighbours 
  ...
  while (queue.length != 0) {
    ...
    for (let i = 0; i < neighbours.length; i++) {
      let currentNeighbour = neighbours[i]; // If node was visited already 
      ...
      currentNeighbour.status = true; // Add reference to previous node, this is to 
